...
import { getWorkflowExecutionsAction } from '@/app/educator/workflows/actions'
import type {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg px-6">
        <SheetHeader>
          ...
          <SheetDescription>
            Detalle de ejecuciones del workflow
          </SheetDescription>
        </SheetHeader>

        {/* Use key to remount content when ID changes, ensuring fresh data load */}
        {open && (
...
function WorkflowDetailContent({ courseWorkflowId }: WorkflowDetailContentProps) {
  ...
  useEffect(() => {
    startTransition(async () => {
      const result = await getWorkflowExecutionsAction(courseWorkflowId)
  ...
  const formatStudentName = (student: { firstName: string | null; lastName: string | null }) => {
        ...
        <TabsContent value="pending" className="mt-4">
          <ScrollArea className="h-[400px] pr-4">
            {isPending ? (
              ...
              <EmptyState
                icon={Clock}
                message="No hay emails programados"
              />
            )}
          </ScrollArea>
        </TabsContent>
        ...
        <TabsContent value="history" className="mt-4">
          <ScrollArea className="h-[400px] pr-4">
            {isPending ? (
              ...
              <EmptyState
                icon={Mail}
                message="No hay emails enviados"
              />
            )}
          </ScrollArea>
        </TabsContent>
...
interface ExecutionCardProps {
  ...
  formatStudentName: (student: { firstName: string | null; lastName: string | null }) => string
