...
export interface AdminMetricCardProps {
  ...
  /** Subtitle text shown below the value (no percentage formatting) */
  subtitle?: string
...
export function AdminMetricCard({
  ...
  const getVariantStyles = () => {
    switch (variant) {
      ...
      case 'warning':
        return {
          container: 'bg-gradient-to-br from-amber-500 to-orange-500 text-white',
          label: 'text-white/80',
          value: 'text-white',
          icon: 'bg-white/20 text-white',
      ...
      default:
        return {
          container: 'bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800',
    ...
    >
      <div className="flex items-start justify-between gap-3 h-full">
        <div className="flex-1 min-w-0 flex flex-col h-full">
          {/* Label */}
          <p className={`text-xs font-medium mb-1 ${styles.label}`}>
            {label}
          </p>

          {/* Value */}
          <p className={`font-bold tracking-tight ${styles.value} ${
            size === 'compact' ? 'text-xl' : 'text-2xl'
          }`}>
            {value}
          </p>

          {/* Subtitle or Trend - always at bottom */}
          {subtitle && (
            <p className={`mt-auto pt-2 text-xs ${styles.label}`}>
              {subtitle}
            </p>
          )}
          {trend && !subtitle && (
            <div className={`flex items-center gap-1 mt-auto pt-2 text-xs font-medium ${getTrendColor()}`}>
              {getTrendIcon()}
              <span>
                {trend.value > 0 ? '+' : ''}
