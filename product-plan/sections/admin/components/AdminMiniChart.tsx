import type { ChartDataPoint } from '@/../product/sections/admin/types'
...
export function AdminMiniChart({
  ...
  const displayValue = formatValue ? formatValue(currentValue) : `${prefix}${currentValue.toLocaleString()}${suffix}`
    ...
    <div className="bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-800 p-4">
      ...
      <div className="relative">
        ...
        >
          {/* Gradient definition */}
          <defs>
            <linearGradient id={`admin-gradient-${color}-${label.replace(/\s/g, '')}`} x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <path
            d={areaPath}
            fill={`url(#admin-gradient-${color}-${label.replace(/\s/g, '')})`}
          />
